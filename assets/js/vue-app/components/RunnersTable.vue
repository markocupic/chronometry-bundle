  ...
  <div class="table-container table-responsive">
    <table id="startlistTable" class="startlist-table table table-dark table-hover">
      ...
      <tr>
        ...
        <th class="align-middle" data-field="status">Status</th>
        <th class="align-middle" data-field="rank">Rang</th>
        <th class="text-center align-middle no-sort"></th>
      </tr>
      </thead>
      <tbody>
      <tr v-for="(runner, index) in store.runners" :key="runner.id" :data-index="index" :data-runnerid="runner.id" :data-number="runner.number" :data-fullname="runner.fullname" :data-category="runner.category" :data-rank="runner.rank" :data-status="runner.status">
        <td class="text-center align-middle status-icon">
          <span :title="runner.status"><i class="fa-solid fa-circle"></i></span>
        </td>
        ...
        <td class="align-middle runningtime" :data-sort="runner.status"><span role="button" @click="store.openModal(index)" title="Datensatz bearbeiten">{{ runner.status }}</span></td>
